...
fcheck_error(){
    ...
    then
        ...
        ${c_echo} -e "`date +"%Y-%m-%d %H:%M:%S"` ${F_RED}Do you want continue? ${F_OFF}[S/N]: ";
        read opcion
        case ${opcion} in
        S|s|Y|y)
...
# Package Instalation.
${c_echo} "Do you want to install necesary packages and remove innecesary packages? [ S/N ]";
read answer
case ${answer} in
    S|s|Y|y) 
...
# Backup files
${c_echo} "Do you want to make backup files? [ S/N ]";
read answer
...
${c_echo} "Do you want to start the hardening? [ S/N ]";
read answer
case ${answer} in
    S|s|Y|y) 
        # Kernel Parámeters.
